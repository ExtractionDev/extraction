...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ATOMIC CLAIM: delete only if still active, and require a row to come back.
    // If two buyers paid for the same listing, only one delete returns a row;
    // the other gets a clear "already sold" instead of both receiving the item.
    ...
    if (!claimed || !claimed.length) {
      // Someone else claimed it first (or it was withdrawn). Buyer already paid
      // on-chain — route to support so the payment can be refunded/resolved.
      return res.status(409).json({ error: 'Listing was already sold or withdrawn. Contact support with TX: ' + signature });
    }
