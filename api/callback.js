...
export default async function handler(req, res) {
  try {
    ...
    // 5. Streak state for DISPLAY only. Actual claiming + crediting happens via
    // the in-game Claim button (action:'claim_streak' in load.js), which enforces
    // a 24h rolling cooldown. Here we just compute what day to show.
    const today = new Date().toISOString().split('T')[0];
    let streak = (player && player.login_streak) || 1;
    let streakBonus = 0; // no auto-credit on login anymore
